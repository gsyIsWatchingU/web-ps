...
- Notes: 已支持本地图片导入并自动生成图片图层，属性面板可完成缩放、旋转、水平翻转、垂直翻转；当前已补齐自由裁切和 `1:1 / 4:5 / 9:16` 快捷裁切。
...
- Notes: 对应 `AC-4`。当前已支持新增文字层、编辑文案、字号、字重、描边、阴影、渐变和背景条，并补齐四类高频电商花字模板。
...
- Notes: 对应 `AC-5`。当前已接入 Fabric 运行时滤镜预览，并补齐美妆、食品、服饰、家居四类电商滤镜预设和一键增强入口。
...
- Notes: 对应 `AC-6`。撤销 / 重做与画布对象回写已落地，当前新增裁切、模板、滤镜预设、视口平移后已通过 `typecheck/build` 工程校验，仍缺“20 次连续操作”的正式自测记录。
...
- Notes: 对应 `AC-7`, `AC-8`。本地草稿恢复与 PNG/JPEG 导出已落地，新增了按比例与版本号命名的导出结果；仍缺导出一致性与异常场景校验。
...
- Owner: frontend
- Depends on: T2, T3
- Acceptance: 支持手型模式平移画布视口，支持自由裁切与 `1:1 / 4:5 / 9:16` 快捷裁切。
- Notes: 视口状态已纳入文档模型，裁切结果会进入导出链路。
...
- Owner: frontend
- Depends on: T5, T6
- Acceptance: 提供四类高频花字模板、四类电商滤镜预设与一键增强。
- Notes: 当前模板以主标题、价格角标、优惠券标签、卖点高亮条为主，滤镜覆盖美妆、食品、服饰、家居。
...
- Notes: 防止运行态与导出态渲染差异，尤其关注裁切区域、电商滤镜预设和模板花字。
