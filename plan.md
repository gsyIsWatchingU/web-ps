...
- 首版对参考图输入采用“仅支持可访问 URL”的约束；本地上传/`data:` 图层在 AI3D 面板中明确提示暂不支持
...
- [src/features/editor/store/useEditorStore.ts](/E:/new-study/web-ps/src/features/editor/store/useEditorStore.ts) 中：
  ...
  - AI3D 任务完成后不修改 `ImageLayer.source`、`crop`、`transform` 或其他画布内容。
- 轮询归 store 持有，不放在组件里：
  - 组件只负责触发和展示状态。
...
- 选中图片图层时显示 `AI3D` 面板；未选中图片时显示引导提示。
- 图片 `source` 为非 `http(s)` 时，AI3D 面板显示不支持提示，提交按钮禁用。
- 未配置 `VITE_AI_BASE_URL / VITE_AI_API_KEY / VITE_AI_MODEL` 时，面板显示明确配置提示。
- 发起 AI3D 时按钮进入 loading，重复点击被阻止。
...
- 首版 AI3D 仅提供“生成并下载 3D 资产”，不做站内预览、导入或回写。
